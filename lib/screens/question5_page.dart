...
import 'package:cosmetics_law/widgets/cosme_name.dart';
import 'package:flutter/material.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

class Question5Page extends ConsumerStatefulWidget {
  const Question5Page({Key? key, required this.title}) : super(key: key);
  ...
  @override
  ConsumerState<Question5Page> createState() => _Question5PageState();
}

class _Question5PageState extends ConsumerState<Question5Page> {
  List<Map> Answer = [
    {
      "firstBox": false,
      "secondBox": false,
        ...
        child: Container(
          ...
          child: Center(
            ...
            child: Column(
              ...
              children: <Widget>[
                Cosme_name_Question(customAnswer: parentSetAnswer)
              ],
            ),
          ),
        ),
      ...
      bottomNavigationBar: QuestionareBottomBar(
        onClickAction: () {
          if (Answer[0]["firstBox"] == true) {
            ref.read(question5Provider.notifier).update((state) => true);
            Navigator.pushNamed(context, '/question6');
          } else {
            ref.read(question5Provider.notifier).update((state) => false);
            ref
